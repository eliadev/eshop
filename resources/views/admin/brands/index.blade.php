					...
					<div class="row">
						<div class="col-md-12">
							<div class="text-sm-right">
								<a href="{{route('brands.create')}}" class="btn btn-info btn-xs"><i class="mdi mdi-plus-circle mr-1"></i> Add New</a>
							</div>
						</div>
					</div>
					<table class="table table-centered table-striped" id="products-datatable">
						...
						@foreach($brands as $brand)
							<tr>
								...
								<td>{{$brand->link}}</td>
								<td>{{$brand->order}}</td>
								<td><a href="{{route('brands.edit', $brand->id)}}" class="btn btn-info btn-xs">Edit</a> 
